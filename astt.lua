...
local InterfaceManager =
	loadLib("https://raw.githubusercontent.com/dawid-scripts/Fluent/master/Addons/InterfaceManager.lua")
...
Players.LocalPlayer.Idled:Connect(function()
	...
	pcall(function()
		game:GetService("ReplicatedStorage").Remotes.Players.prevent_afk:FireServer()
	end)
end)
...
local traitMaps = {}
local prismMaps = {}
local joinableMapTree = {}
local joinableWorlds = {}
local gearFarmMaps = {}
local unitFarmMaps = {}
...
local function ensureEventModules()
	if eventModulesLoaded then
		return true
	end
	local client = Players.LocalPlayer.PlayerScripts:FindFirstChild("Client")
	local events = client and client:FindFirstChild("Events")
	if not events then
		return false
	end
	eventModules = {}
	local okCrusader, Crusader = pcall(function()
		return require(events:FindFirstChild("Crusader"))
	end)
	if okCrusader and type(Crusader) == "table" and type(Crusader.rewards) == "table" then
		local acts = {}
		for act, rewards in ipairs(Crusader.rewards) do
			acts[act] = { rewards = rewards, boss = Crusader.bosses and Crusader.bosses[act] }
		end
		table.insert(eventModules, { name = "Crusader Event", world = "Crusader Desert", acts = acts })
	end
	local okCapsule, Capsule = pcall(function()
		return require(events:FindFirstChild("Capsule"))
	end)
	if okCapsule and type(Capsule) == "table" and type(Capsule.rewards) == "table" then
		table.insert(eventModules, {
			name = "Capsule Event",
			world = "The Royal Capital",
			acts = { { rewards = Capsule.rewards, boss = "Asto" } },
		})
	end
	eventModulesLoaded = #eventModules > 0
	return eventModulesLoaded
end

local function rebuildJoinableData()
	joinableMapTree = {}
	gearFarmMaps = {}
	unitFarmMaps = {}
	prismMaps = {}
	...
	for _, ev in ipairs(eventModules) do
		local node = { Event = { Normal = {} } }
		for act, data in pairs(ev.acts) do
			node.Event.Normal[act] = {
				world = ev.world,
				omitDifficulty = true,
	...
	local okCorr, Corrupted = pcall(function()
		return require(Players.LocalPlayer.PlayerScripts.Client.Corrupted_Stages)
	end)
	if okCorr and type(Corrupted) == "table" and type(Corrupted.stages) == "table" then
		for _, stage in pairs(Corrupted.stages) do
			if type(stage) == "table" and stage.name and type(stage.rewards) == "table" then
				joinableMapTree[stage.name] = joinableMapTree[stage.name] or {}
				joinableMapTree[stage.name].Corrupted = joinableMapTree[stage.name].Corrupted or {}
				joinableMapTree[stage.name].Corrupted.Normal = joinableMapTree[stage.name].Corrupted.Normal or {}
				joinableMapTree[stage.name].Corrupted.Normal[1] = {
					world = stage.name,
	...
	joinableWorlds = {}
	for world in pairs(joinableMapTree) do
		if next(joinableMapTree[world]) then
			table.insert(joinableWorlds, world)
		end
	end
	table.sort(joinableWorlds)
	gearFarmTargets = {}
	unitFarmTargets = {}
	for target in pairs(gearFarmMaps) do
		table.insert(gearFarmTargets, target)
	end
	for target in pairs(unitFarmMaps) do
		table.insert(unitFarmTargets, target)
	end
	table.sort(gearFarmTargets)
	table.sort(unitFarmTargets)
end
...
if isLobby then
	if commonWorldsLoaded then
		...
		pcall(function()
			local Corr = require(Players.LocalPlayer.PlayerScripts.Client.Corrupted_Stages)
		...
		local function loadCache()
			...
			if isfile and readfile and isfile(basePath .. "/MapsCache.json") then
				local succ2, data2 = pcall(function()
					return HttpService:JSONDecode(readfile(basePath .. "/MapsCache.json"))
				end)
...
if not isfile("as_free/Language.txt") then
	...
	btnVn.MouseButton1Click:Connect(function()
		...
		pcall(function()
			writefile("as_free/Language.txt", "VN")
		end)
		languageSelected = true
	end)
...
local Locales = {
	EN = {
		...
		TabAutoJoinMap = "Auto Join Map",
		TabFarmGear = "Farm Gear",
		TabFarmUnit = "Farm Unit",
		TabPriority = "Priority Settings",
		TabMaps = "Trait Maps",
		TabFarmPrism = "Farm Prism",
		TabEvo = "Evo & Craft",
		...
		PrismCfg = "Auto Prism Farm",
		PrismCfgD = "Farm Prism from Corrupted stage (Shibuya Destroyed) up to daily limit.",
		PrismTog = "ENABLE Auto Prism",
		PrismTogD = "Auto farms Prism map until cap.",
		CorruptedMod = "Corrupted Modifiers",
		CorruptedModD = "Select modifiers for Corrupted mode. Empty = none. Crimson needs 3.",
		...
		AF_AutoMap = "Enable Auto Join Map",
		AF_AutoMapD = "Automatically joins the selected map when higher-priority tasks are idle.",
		AF_MapWorld = "Story / World",
		AF_MapWorldD = "Select a story world.",
		AF_MapMode = "Mode",
		AF_MapModeD = "Available modes for the selected world.",
		AF_MapDiff = "Difficulty",
		AF_MapDiffD = "Available difficulties for the selected mode.",
		AF_MapAct = "Map / Act",
		AF_MapActD = "Available maps in the selected story.",
		FarmGearTarget = "Target Gear",
		FarmGearTargetD = "Select gear; its drop map is detected automatically.",
		FarmGearQty = "Quantity to Farm",
		FarmGearQtyD = "Number of new copies to obtain.",
		FarmGearQueue = "Gear Farm Queue",
		FarmGearQueueD = "No gear queued.",
		FarmGearAdd = "Add to Queue",
		FarmGearAddD = "Add the selected gear, quantity, and difficulty.",
		FarmGearClear = "Clear Queue",
		FarmGearClearD = "Remove all queued gear.",
		FarmUnitTarget = "Target Unit",
		FarmUnitTargetD = "Select a drop unit; its map is detected automatically.",
		FarmDifficulty = "Farm Difficulty",
		FarmDifficultyD = "Hard usually has a higher drop chance.",
		FarmGearToggle = "Enable Auto Farm Gear",
		FarmGearToggleD = "Automatically joins and replays the selected gear map.",
		FarmUnitToggle = "Enable Auto Farm Unit",
		FarmUnitToggleD = "Automatically joins and replays the selected unit map.",
		FarmTargetInfo = "Target Map",
		FarmTargetWaiting = "Select a valid target.",
		...
		Pri4 = "Priority 4",
		Pri4D = "Runs last.",
		Pri5 = "Priority 5 (Prism)",
		Pri5D = "Runs after Priority 4. Default Auto Prism.",
	...
	VN = {
		...
		WinSub = "Anime Squadron",
		TabAutoFarm = "Auto Farm",
		TabAutoJoinMap = "Tự động Vào Map",
		TabFarmGear = "Farm Gear",
		TabFarmUnit = "Farm Unit",
		...
		PrismCfg = "Auto Farm Prism",
		PrismCfgD = "Farm Prism từ Corrupted stage (Shibuya Destroyed) tới giới hạn ngày.",
		PrismTog = "BẬT Auto Prism",
		PrismTogD = "Tự farm map Prism tới cap.",
		CorruptedMod = "Corrupted Modifiers",
		CorruptedModD = "Chọn modifier cho Corrupted. Trống = không mod. Crimson cần 3.",
		...
		AF_AutoMap = "Bật Tự động Vào Map",
		AF_AutoMapD = "Tự vào map đã chọn khi các tác vụ ưu tiên cao hơn đang rảnh.",
		AF_MapWorld = "Story / World",
		AF_MapWorldD = "Chọn khu vực story.",
		AF_MapMode = "Chế độ",
		AF_MapModeD = "Các chế độ có trong story đã chọn.",
		AF_MapDiff = "Độ khó",
		AF_MapDiffD = "Các độ khó có trong chế độ đã chọn.",
		AF_MapAct = "Map / Act",
		AF_MapActD = "Danh sách map thuộc story đã chọn.",
		FarmGearTarget = "Gear cần Farm",
		FarmGearTargetD = "Chọn gear, script tự tìm map rơi gear đó.",
		FarmGearQty = "Số lượng cần Farm",
		FarmGearQtyD = "Số bản gear mới cần kiếm.",
		FarmGearQueue = "Hàng đợi Farm Gear",
		FarmGearQueueD = "Chưa có gear trong hàng đợi.",
		FarmGearAdd = "Thêm vào Hàng đợi",
		FarmGearAddD = "Thêm gear, số lượng và độ khó đã chọn.",
		FarmGearClear = "Xoá Hàng đợi",
		FarmGearClearD = "Xoá toàn bộ gear đang chờ farm.",
		FarmUnitTarget = "Unit cần Farm",
		FarmUnitTargetD = "Chọn unit rơi từ map, script tự tìm đúng map.",
		FarmDifficulty = "Độ khó Farm",
		FarmDifficultyD = "Hard thường có tỉ lệ rơi cao hơn.",
		FarmGearToggle = "Bật Auto Farm Gear",
		FarmGearToggleD = "Tự vào và replay map rơi gear đã chọn.",
		FarmUnitToggle = "Bật Auto Farm Unit",
		FarmUnitToggleD = "Tự vào và replay map rơi unit đã chọn.",
		FarmTargetInfo = "Map Mục tiêu",
		FarmTargetWaiting = "Hãy chọn mục tiêu hợp lệ.",
		...
		Pri5 = "Ưu tiên 5 (Prism)",
		Pri5D = "Chạy sau Ưu tiên 4. Mặc định Auto Prism.",
...
local Tabs = {
	...
	AutoJoinMap = Window:AddTab({ Title = L.TabAutoJoinMap, Icon = "map" }),
	FarmGear = Window:AddTab({ Title = L.TabFarmGear, Icon = "shield" }),
	FarmUnit = Window:AddTab({ Title = L.TabFarmUnit, Icon = "users" }),
	Priority = Window:AddTab({ Title = L.TabPriority, Icon = "list-ordered" }),
	Maps = Window:AddTab({ Title = L.TabMaps, Icon = "map" }),
	FarmPrism = Window:AddTab({ Title = L.TabFarmPrism, Icon = "gem" }),
...
local prismMapConfigs = {}
Tabs.FarmPrism:AddParagraph({ Title = L.PrismCfg, Content = L.PrismCfgD })
if #prismMaps == 0 then
	Tabs.FarmPrism:AddParagraph({ Title = L.MapNoData, Content = "No Prism data - run in Lobby" })
else
	for i, map in ipairs(prismMaps) do
		local capStr = get_cap_string(map.mode, map.world, map.act, map.reward)
		local titleStr = string.format("[%s] %s (Act %d) - %s", map.mode, map.world, map.act, map.reward)
		local paragraph = Tabs.FarmPrism:AddParagraph({ Title = titleStr, Content = L.MapTrack })
		local input = Tabs.FarmPrism:AddInput("PrismPriority_" .. i, {
			Title = L.MapPriority,
			Description = L.MapPriorityD,
			Default = tostring(#traitMaps + i),
			Numeric = true,
			Finished = false,
		})
		local diff = Tabs.FarmPrism:AddDropdown("PrismDiff_" .. i, {
			Title = L.MapDiff,
			Description = L.MapDiffD,
			Values = { "Normal" },
			Multi = false,
...
local ToggleAutoPlay =
	Tabs.Ingame:AddToggle("AutoPlayToggle", { Title = L.IgAutoPlay, Description = L.IgAutoPlayD, Default = false })
local ToggleLeave =
	Tabs.Ingame:AddToggle("AutoLeaveToggle", { Title = L.IgAutoLeave, Description = L.IgAutoLeaveD, Default = false })
local ToggleLeaveBase = Tabs.Ingame:AddToggle(
	"AutoLeaveBaseFailsafe",
	{ Title = L.IgAutoLeaveBase, Description = L.IgAutoLeaveBaseD, Default = false }
)
local ToggleReplay = Tabs.Ingame:AddToggle(
	"AutoReplayToggle",
	{ Title = L.IgAutoReplay, Description = L.IgAutoReplayD, Default = false }
)
local ToggleReplayAtWave =
	Tabs.Ingame:AddToggle("AutoReplayAtWave", { Title = L.IgReplayInf, Description = L.IgReplayInfD, Default = false })
local InputReplayWave = Tabs.Ingame:AddInput(
	"ReplayWaveTarget",
	{ Title = L.IgWaveTarget, Description = L.IgWaveTargetD, Default = "30", Numeric = true, Finished = false }
)
local ToggleSpeed =
...
local ToggleSniperSync =
	Tabs.Ingame:AddToggle("AutoSniperSync", { Title = L.IgSniperTog, Description = L.IgSniperTogD, Default = false })
...
local DropdownEvoTarget = Tabs.EvoCraft:AddDropdown(
	"EvoTarget",
	{ Title = L.EvoTarget, Description = L.EvoTargetD, Values = { initEvo }, Multi = false, Default = 1 }
)
...
local ToggleAutoEvo =
	Tabs.EvoCraft:AddToggle("AutoEvo", { Title = L.EvoToggle, Description = L.EvoToggleD, Default = false })

Tabs.EvoCraft:AddParagraph({ Title = "---", Content = "" })

local initCraft = getSavedTarget("CraftTarget", "(Loading Craftables...)")
local DropdownCraftTarget = Tabs.EvoCraft:AddDropdown(
	"CraftTarget",
...
task.spawn(function()
	...
	if #craftTargets == 0 then
		table.insert(craftTargets, "(None)")
	end
	...
	if currentCraft then
		DropdownCraftTarget:SetValue(currentCraft)
	end
end)
local InputCraftQty = Tabs.EvoCraft:AddInput(
	"CraftQty",
	{ Title = L.CraftQty, Description = L.CraftQtyD, Default = "1", Numeric = true, Finished = false }
)
...
_G.AnimeSquadron_UpdateCraftQueueUI = function()
	...
	if isfile and writefile then
		pcall(function()
			writefile(
				basePath .. "/CraftQueue.json",
				game:GetService("HttpService"):JSONEncode(_G.AnimeSquadron_CraftQueue)
			)
		end)
	end
end
...
local ToggleAutoCraft =
	Tabs.EvoCraft:AddToggle("AutoCraft", { Title = L.CraftToggle, Description = L.CraftToggleD, Default = false })
Tabs.AutoReroll:AddParagraph({ Title = L.RerollCfg, Content = L.RerollCfgD })

local DropdownRerollUnit = Tabs.AutoReroll:AddDropdown(
	"RerollUnit",
...
local DropdownLockedStats = Tabs.AutoReroll:AddDropdown("LockedStats", {
	Title = L.RerollLock,
	Description = L.RerollLockD,
	Values = { "SSS", "SS", "S+", "S", "A+", "A", "B+", "B", "C+", "C", "C-" },
	Multi = true,
	Default = { ["SSS"] = true },
})

local ToggleAutoReroll =
...
local DropdownMerchantItem = Tabs.ShopUpgrade:AddDropdown(
	"MerchantItem",
	{ Title = L.MerchItem, Description = L.MerchItemD, Values = { "(Loading Items...)" }, Multi = true, Default = {} }
)

task.spawn(function()
	local allMerchantItems = loadShopItems("Merchant")
	if type(allMerchantItems) ~= "table" then
		allMerchantItems = {}
	end
	...
	if isLobby then
		pcall(function()
			...
			local blacklist = {}
			if get then
				local succ1, raid = pcall(function()
					return get:InvokeServer("gt_city_raid")
				end)
				...
				local succ2, event = pcall(function()
					return get:InvokeServer("baras_event")
				end)
				...
				if folder then
					for _, v in ipairs(folder:GetChildren()) do
						...
						if whitelist[name] then
							if not table.find(tempItems, name) then
								table.insert(tempItems, name)
							end
						else
							if
								not blacklist[name]
								and not string.find(name, "XP")
								and not string.find(name, "Coin")
							then
								if not table.find(tempItems, name) then
									table.insert(tempItems, name)
								end
	...
	if #allMerchantItems == 0 then
		table.insert(allMerchantItems, "(Empty)")
	end
	local currentMerchant = Options.MerchantItem and Options.MerchantItem.Value
	DropdownMerchantItem:SetValues(allMerchantItems)
	if type(currentMerchant) == "table" then
		DropdownMerchantItem:SetValue(currentMerchant)
	end
end)
local ToggleAutoBuyMerchant =
	Tabs.ShopUpgrade:AddToggle("AutoBuyMerchant", { Title = L.MerchTog, Description = L.MerchTogD, Default = false })

local DropdownRaidShopItem = Tabs.ShopUpgrade:AddDropdown(
	"RaidShopItems",
	{ Title = L.RaidItem, Description = L.RaidItemD, Values = { "(Waiting...)" }, Multi = true, Default = {} }
)
local ToggleAutoBuyRaid =
	Tabs.ShopUpgrade:AddToggle("AutoBuyRaid", { Title = L.RaidTog, Description = L.RaidTogD, Default = false })

local DropdownEventShopItem = Tabs.ShopUpgrade:AddDropdown(
	"EventShopItems",
	{ Title = L.EventItem, Description = L.EventItemD, Values = { "(Waiting...)" }, Multi = true, Default = {} }
)
local ToggleAutoBuyEvent =
	Tabs.ShopUpgrade:AddToggle("AutoBuyEvent", { Title = L.EventTog, Description = L.EventTogD, Default = false })

task.spawn(function()
	pcall(function()
		local raidItems = loadShopItems("Raid")
		if #raidItems > 0 then
			DropdownRaidShopItem:SetValues(raidItems)
		end
		local rCurrent = Options.RaidShopItems and Options.RaidShopItems.Value
		if type(rCurrent) == "table" then
			DropdownRaidShopItem:SetValue(rCurrent)
		end

		local eventItems = loadShopItems("Event")
		if #eventItems > 0 then
			DropdownEventShopItem:SetValues(eventItems)
		end
		local eCurrent = Options.EventShopItems and Options.EventShopItems.Value
		if type(eCurrent) == "table" then
			DropdownEventShopItem:SetValue(eCurrent)
		end
	end)
end)
...
local DropdownPerkTarget = Tabs.ShopUpgrade:AddDropdown("PerkTarget", {
	...
	Values = { "health", "yen_generation", "yen_max" },
	Multi = false,
	Default = 1,
})
local ToggleAutoPerk =
	Tabs.ShopUpgrade:AddToggle("AutoPerk", { Title = L.PerkTog, Description = L.PerkTogD, Default = false })
Tabs.Claims:AddParagraph({ Title = L.ClaimCfg, Content = L.ClaimCfgD })
local ToggleAutoPass =
	Tabs.Claims:AddToggle("AutoPass", { Title = L.ClaimPass, Description = L.ClaimPassD, Default = false })
local ToggleAutoMilestones =
	Tabs.Claims:AddToggle("AutoMilestones", { Title = L.ClaimMile, Description = L.ClaimMileD, Default = false })
local ToggleAutoDiscovery =
	Tabs.Claims:AddToggle("AutoDiscovery", { Title = L.ClaimIdx, Description = L.ClaimIdxD, Default = false })
							...
							if string.sub(txt, 1, 2) == "- " then
								...
								if not string.find(possibleCode, " ") then
									...
									for _, c in ipairs(codes) do
										if c == possibleCode then
											alreadyHas = true
											break
										end
									end
									if not alreadyHas then
										table.insert(codes, possibleCode)
									end
								end
							end
						end
...
local function saveSessionStats()
	if writefile then
		pcall(function()
			writefile(statsFileName, game:GetService("HttpService"):JSONEncode(SessionStats))
		end)
	end
end
...
local function updateStatsUI()
	if StatsParagraph then
		...
		local function getCurrentAmount(name)
			if not (util and util.data) then
				return 0
			end
			if name == "Gold" then
				return util.data.stats and util.data.stats.Gold or 0
			end
			if name == "Gems" then
				return util.data.stats and util.data.stats.Gems or 0
			end
			if name == "Trait Shards" then
				return util.data.stats and util.data.stats["Trait Shards"] or 0
			end
			if name == "Perfect Cubes" then
				return util.data.stats and util.data.stats["Perfect Cubes"] or 0
			end
			if name == "Reroll Cubes" then
				return util.data.stats and util.data.stats["Reroll Cubes"] or 0
			end
			return 0
		end
...
local function loadSessionStats()
	if isfile and readfile and isfile(statsFileName) then
		local s, res = pcall(function()
			return game:GetService("HttpService"):JSONDecode(readfile(statsFileName))
		end)
...
loadSessionStats()
local friendToggle =
	Tabs.AutoFarm:AddToggle("FriendsOnly", { Title = L.AF_Friend, Description = L.AF_FriendD, Default = true })
local AutoClaimDaily =
	Tabs.AutoFarm:AddToggle("AutoClaimDaily", { Title = L.AF_Daily, Description = L.AF_DailyD, Default = false })
local AutoClaimBundle =
	Tabs.AutoFarm:AddToggle("AutoClaimBundle", { Title = L.AF_Bun, Description = L.AF_BunD, Default = false })
local AutoQuest =
	Tabs.AutoFarm:AddToggle("AutoQuest", { Title = L.AF_Quest, Description = L.AF_QuestD, Default = false })
local AutoPrism =
	Tabs.AutoFarm:AddToggle("AutoPrism", { Title = L.PrismTog, Description = L.PrismTogD, Default = false })
local AutoFarmModifiers = Tabs.AutoFarm:AddDropdown("AutoFarmModifiers", {
	Title = L.CorruptedMod,
	Description = L.CorruptedModD,
	Values = { "+50% DMG", "-30% YEN", "+50% FASTER ENEMY SPAWN", "+25% DODGE" },
	Multi = true,
	Default = {},
})
local AutoJoinWorld = Tabs.AutoJoinMap:AddDropdown("AutoJoinWorld", {
	Title = L.AF_MapWorld,
	Description = L.AF_MapWorldD,
	Values = #joinableWorlds > 0 and joinableWorlds or { "(No worlds available)" },
	Multi = false,
	Default = 1,
})
local AutoJoinMode = Tabs.AutoJoinMap:AddDropdown(
	"AutoJoinMode",
	{ Title = L.AF_MapMode, Description = L.AF_MapModeD, Values = { "Story" }, Multi = false, Default = 1 }
)
local AutoJoinDifficulty = Tabs.AutoJoinMap:AddDropdown(
	"AutoJoinDifficulty",
	{ Title = L.AF_MapDiff, Description = L.AF_MapDiffD, Values = { "Normal" }, Multi = false, Default = 1 }
)
local AutoJoinAct = Tabs.AutoJoinMap:AddDropdown(
	"AutoJoinAct",
	{ Title = L.AF_MapAct, Description = L.AF_MapActD, Values = { "Act 1" }, Multi = false, Default = 1 }
)
local AutoJoinMap =
	Tabs.AutoJoinMap:AddToggle("AutoJoinMap", { Title = L.AF_AutoMap, Description = L.AF_AutoMapD, Default = false })

local function getCorruptedModifiers()
	if Options.AutoFarmModifiers and type(Options.AutoFarmModifiers.Value) == "table" then
		local mods = {}
		for modName, enabled in pairs(Options.AutoFarmModifiers.Value) do
			if enabled then
...
local function sortedKeys(data, formatKey)
	local values = {}
	for key in pairs(data or {}) do
		table.insert(values, formatKey and formatKey(key) or key)
	end
	table.sort(values, function(a, b)
		local aNumber = tonumber(string.match(tostring(a), "%d+"))
		local bNumber = tonumber(string.match(tostring(b), "%d+"))
...
local function moveValueFirst(values, target)
	local index = table.find(values, target)
	if index and index > 1 then
		table.remove(values, index)
		table.insert(values, 1, target)
	end
end
...
local function refreshAutoJoinMapControls()
	...
	refreshingAutoJoinMap = true

	local world = Options.AutoJoinWorld.Value
	...
	local difficultyData = modeData[Options.AutoJoinDifficulty.Value] or {}
	...
	refreshingAutoJoinMap = false
end

refreshAutoJoinMapControls()

local FarmGearTarget = Tabs.FarmGear:AddDropdown("FarmGearTarget", {
	Title = L.FarmGearTarget,
	Description = L.FarmGearTargetD,
	Values = #gearFarmTargets > 0 and gearFarmTargets or { "(No gear found)" },
	Multi = false,
	Default = 1,
})
local FarmGearDifficulty = Tabs.FarmGear:AddDropdown("FarmGearDifficulty", {
	Title = L.FarmDifficulty,
	Description = L.FarmDifficultyD,
	Values = { "Hard", "Normal" },
	Multi = false,
	Default = 1,
})
local FarmGearQty = Tabs.FarmGear:AddInput(
	"FarmGearQty",
	{ Title = L.FarmGearQty, Description = L.FarmGearQtyD, Default = "1", Numeric = true, Finished = false }
)
local FarmGearInfo = Tabs.FarmGear:AddParagraph({ Title = L.FarmTargetInfo, Content = L.FarmTargetWaiting })
local FarmGearQueueInfo = Tabs.FarmGear:AddParagraph({ Title = L.FarmGearQueue, Content = L.FarmGearQueueD })

local function getOwnedGearCount(targetName)
	local util
	pcall(function()
		util = require(Players.LocalPlayer.PlayerScripts.Client.Utility)
	end)
	local count = 0
	for _, gear in pairs(util and util.data and util.data.gear or {}) do
		if type(gear) == "table" and gear.name == targetName then
			count = count + 1
		end
	end
	return count
...
_G.AnimeSquadron_GearFarmQueue = {}
local gearFarmQueueFile = basePath .. "/GearFarmQueue.json"
if isfile and readfile and isfile(gearFarmQueueFile) then
	pcall(function()
		local data = HttpService:JSONDecode(readfile(gearFarmQueueFile))
		if type(data) == "table" then
			_G.AnimeSquadron_GearFarmQueue = data
		end
	end)
end

local function saveGearFarmQueue()
	if writefile then
		pcall(function()
			writefile(gearFarmQueueFile, HttpService:JSONEncode(_G.AnimeSquadron_GearFarmQueue))
		end)
	end
end

local function updateGearFarmQueueUI()
	if #_G.AnimeSquadron_GearFarmQueue == 0 then
		FarmGearQueueInfo:SetDesc(L.FarmGearQueueD)
		return
	end
	local lines = {}
	for i, taskData in ipairs(_G.AnimeSquadron_GearFarmQueue) do
		local owned = getOwnedGearCount(taskData.name)
		local gained = math.max(0, owned - (tonumber(taskData.startCount) or owned))
		table.insert(
			lines,
			string.format(
				"%d. %s x%d [%d/%d] (%s)",
	...
	FarmGearQueueInfo:SetDesc(table.concat(lines, "\n"))
end

Tabs.FarmGear:AddButton({
	Title = L.FarmGearAdd,
	Description = L.FarmGearAddD,
	Callback = function()
		local target = Options.FarmGearTarget.Value
		local quantity = math.max(1, math.floor(tonumber(Options.FarmGearQty.Value) or 1))
		local difficulty = Options.FarmGearDifficulty.Value
		if not gearFarmMaps[target] then
			return
		end
		for _, taskData in ipairs(_G.AnimeSquadron_GearFarmQueue) do
			if taskData.name == target and taskData.difficulty == difficulty then
				taskData.qty = taskData.qty + quantity
				saveGearFarmQueue()
				updateGearFarmQueueUI()
				return
			end
		end
		table.insert(_G.AnimeSquadron_GearFarmQueue, {
			name = target,
			qty = quantity,
			difficulty = difficulty,
			startCount = getOwnedGearCount(target),
		})
		saveGearFarmQueue()
		updateGearFarmQueueUI()
	end,
...
Tabs.FarmGear:AddButton({
	Title = L.FarmGearClear,
	Description = L.FarmGearClearD,
	Callback = function()
		_G.AnimeSquadron_GearFarmQueue = {}
		saveGearFarmQueue()
		updateGearFarmQueueUI()
...
local function getActiveGearFarmTask()
	local changed = false
	while _G.AnimeSquadron_GearFarmQueue[1] do
		local taskData = _G.AnimeSquadron_GearFarmQueue[1]
		local owned = getOwnedGearCount(taskData.name)
		taskData.startCount = tonumber(taskData.startCount) or owned
		taskData.qty = math.max(1, tonumber(taskData.qty) or 1)
		if owned - taskData.startCount >= taskData.qty then
			table.remove(_G.AnimeSquadron_GearFarmQueue, 1)
			changed = true
		else
			if changed then
				saveGearFarmQueue()
...
updateGearFarmQueueUI()
local AutoFarmGear = Tabs.FarmGear:AddToggle(
	"AutoFarmGear",
	{ Title = L.FarmGearToggle, Description = L.FarmGearToggleD, Default = false }
)

local FarmUnitTarget = Tabs.FarmUnit:AddDropdown("FarmUnitTarget", {
	Title = L.FarmUnitTarget,
	Description = L.FarmUnitTargetD,
	Values = #unitFarmTargets > 0 and unitFarmTargets or { "(No units found)" },
	Multi = false,
	Default = 1,
})
local FarmUnitDifficulty = Tabs.FarmUnit:AddDropdown("FarmUnitDifficulty", {
	Title = L.FarmDifficulty,
	Description = L.FarmDifficultyD,
	Values = { "Hard", "Normal" },
	Multi = false,
...
local FarmUnitInfo = Tabs.FarmUnit:AddParagraph({ Title = L.FarmTargetInfo, Content = L.FarmTargetWaiting })
local AutoFarmUnit = Tabs.FarmUnit:AddToggle(
	"AutoFarmUnit",
	{ Title = L.FarmUnitToggle, Description = L.FarmUnitToggleD, Default = false }
)

local function resolveFarmMap(targetMaps, target, difficulty)
	local maps = targetMaps[target]
	if not maps then
		return nil
	end
...
local function formatFarmMap(map)
	if not map then
		return L.FarmTargetWaiting
	end
	local chance = map.chance and (" | Drop: " .. tostring(map.chance)) or ""
...
task.spawn(function()
	task.wait(5)
	local changed = ensureEventModules()
	if changed then
		rebuildJoinableData()
	end
	pcall(function()
		if #joinableWorlds > 0 then
			AutoJoinWorld:SetValues(joinableWorlds)
		end
	end)
	pcall(function()
		if #gearFarmTargets > 0 then
			FarmGearTarget:SetValues(gearFarmTargets)
		end
	end)
	pcall(function()
		if #unitFarmTargets > 0 then
			FarmUnitTarget:SetValues(unitFarmTargets)
		end
	end)
	pcall(refreshAutoJoinMapControls)
	pcall(refreshFarmTargetInfo)
end)
local AutoToggle =
...
pcall(function()
	local promptOverlay =
		game:GetService("CoreGui"):WaitForChild("RobloxPromptGui", 10):WaitForChild("promptOverlay", 10)
...
local PriorityList =
	{ "Auto Quest", "Auto Farm Gear", "Auto Farm Unit", "Auto Craft", "Auto Evo", "Auto Reroll", "Auto Prism", "None" }
Tabs.Priority:AddDropdown(
	"Priority1",
	{ Title = L.Pri1, Description = L.Pri1D, Values = PriorityList, Multi = false, Default = "Auto Quest" }
)
Tabs.Priority:AddDropdown(
	"Priority2",
	{ Title = L.Pri2, Description = L.Pri2D, Values = PriorityList, Multi = false, Default = "Auto Craft" }
)
Tabs.Priority:AddDropdown(
	"Priority3",
	{ Title = L.Pri3, Description = L.Pri3D, Values = PriorityList, Multi = false, Default = "Auto Evo" }
)
Tabs.Priority:AddDropdown(
	"Priority4",
	{ Title = L.Pri4, Description = L.Pri4D, Values = PriorityList, Multi = false, Default = "Auto Reroll" }
)
Tabs.Priority:AddDropdown(
	"Priority5",
	{ Title = L.Pri5, Description = L.Pri5D, Values = PriorityList, Multi = false, Default = "Auto Prism" }
)

Tabs.PartyMulti:AddParagraph({ Title = L.PtCfg, Content = L.PtCfgD })
local TogglePartyMode =
	Tabs.PartyMulti:AddToggle("PartyMode", { Title = L.PtTog, Description = L.PtTogD, Default = false })
local DropdownPartyRole = Tabs.PartyMulti:AddDropdown(
	"PartyRole",
	{ Title = L.PtRole, Description = L.PtRoleD, Values = { "Host", "Member" }, Multi = false, Default = "Host" }
)
local TogglePartyLeaveSync =
	Tabs.PartyMulti:AddToggle("PartyLeaveSync", { Title = L.PtSync, Description = L.PtSyncD, Default = true })

Tabs.PartyMulti:AddParagraph({ Title = L.PtHostCfg, Content = L.PtHostCfgD })
local InputMember1 = Tabs.PartyMulti:AddInput(
	"PartyMember1",
	{ Title = L.PtM1, Description = L.PtM1D, Default = "", Numeric = false, Finished = false }
)
local InputMember2 = Tabs.PartyMulti:AddInput(
	"PartyMember2",
	{ Title = L.PtM2, Description = L.PtM2D, Default = "", Numeric = false, Finished = false }
)
local InputMember3 = Tabs.PartyMulti:AddInput(
	"PartyMember3",
	{ Title = L.PtM3, Description = L.PtM3D, Default = "", Numeric = false, Finished = false }
)
local SliderHostTimeout = Tabs.PartyMulti:AddSlider(
	"HostWaitTimeout",
	{ Title = L.PtTime, Description = L.PtTimeD, Default = 5, Min = 1, Max = 10, Rounding = 0 }
)

Tabs.PartyMulti:AddParagraph({ Title = L.PtMemCfg, Content = L.PtMemCfgD })
local InputHostName = Tabs.PartyMulti:AddInput(
	"PartyHostName",
	{ Title = L.PtHostName, Description = L.PtHostNameD, Default = "", Numeric = false, Finished = false }
)
...
local function resolveUsernameToID(username)
	...
	if _G.AnimeSquadron_UserCache[lower] then
		return _G.AnimeSquadron_UserCache[lower]
	end
...
Tabs.Settings:AddSlider(
	"StuckTimeout",
	{ Title = L.StuckTimeout, Description = L.StuckTimeoutD, Default = 60, Min = 30, Max = 300, Rounding = 0 }
)
...
for name, option in pairs(Fluent.Options) do
	option:OnChanged(function()
		if name == "AutoJoinWorld" or name == "AutoJoinMode" or name == "AutoJoinDifficulty" then
			refreshAutoJoinMapControls()
		elseif
			name == "FarmGearTarget"
			or name == "FarmGearDifficulty"
			or name == "FarmUnitTarget"
			or name == "FarmUnitDifficulty"
		then
			refreshFarmTargetInfo()
		end
...
if isLobby then
	...
	local function joinRoom(act, diff, mode, world, rewards, capStr, maxCap, capType, boss, modifiers)
		if Options.PartyMode and Options.PartyMode.Value then
			if Options.PartyRole.Value == "Member" then
				...
				local currentRoom = activeRooms[hostId]
					or activeRooms[tonumber(hostId)]
					or activeRooms[tostring(hostId)]
				if currentRoom then
					Fluent:Notify({ Title = "Party System", Content = "Joining Host's Room!", Duration = 3 })
					if join_remote then
						pcall(function()
							join_remote:InvokeServer(hostId)
						end)
					end
					task.wait(10)
					return true
		...
		if isfile and writefile then
			if capStr and maxCap then
				...
				if isfile(basePath .. "/CurrentTarget.json") and delfile then
					pcall(function()
						delfile(basePath .. "/CurrentTarget.json")
					end)
				end
			end
		end
		...
		if diff then
			roomData.difficulty = diff
		end
		if (not modifiers or type(modifiers) ~= "table" or #modifiers == 0) and Options.AutoFarmModifiers then
			modifiers = getCorruptedModifiers()
		end
		if modifiers and type(modifiers) == "table" and #modifiers > 0 then
			roomData.modifiers = modifiers
		end
		local success, err = create_room:InvokeServer(roomData)

		if success then
			if Options.PartyMode and Options.PartyMode.Value and Options.PartyRole.Value == "Host" then
				local membersToWait = {}
				local m1 = Options.PartyMember1 and Options.PartyMember1.Value
				if m1 and m1 ~= "" then
					table.insert(membersToWait, { name = m1, key = "PartyMember1" })
				end
				local m2 = Options.PartyMember2 and Options.PartyMember2.Value
				if m2 and m2 ~= "" then
					table.insert(membersToWait, { name = m2, key = "PartyMember2" })
				end
				local m3 = Options.PartyMember3 and Options.PartyMember3.Value
				if m3 and m3 ~= "" then
					table.insert(membersToWait, { name = m3, key = "PartyMember3" })
				end
				...
				if next(memberData) then
					local waitingNames = {}
					for _, data in pairs(memberData) do
						table.insert(waitingNames, data.name)
					end
					Fluent:Notify({
						Title = "Party System",
					...
					if #missingMembers > 0 then
						local msg = "Members failed to join (Crashed): "
							.. table.concat(missingMembers, ", ")
							.. ". They have been REMOVED from the Wait List."
	...
	local function getSortedValidTraitMaps()
		...
		table.sort(validMaps, function(a, b)
			return a.priority < b.priority
		end)
		return validMaps
	end
	local function getSortedValidPrismMaps()
		local validMaps = {}
		for i, cfg in ipairs(prismMapConfigs) do
			...
			if prio > 0 then
				validMaps[#validMaps + 1] = {
					cfg = cfg,
					priority = prio,
					difficulty = Options["PrismDiff_" .. i].Value,
				}
			end
		end
		table.sort(validMaps, function(a, b)
	...
	task.spawn(function()
		...
		while true do
				...
				if util and util.data then
					if util.data.items then
						for k, v in pairs(util.data.items) do
							if string.lower(k) == string.lower(matName) then
								have = have + v
							end
						end
					end
					...
					if v.progress >= v.required then
						...
						if string.find(lowerName, "summon") then
							...
							if diff > 0 then
								table.insert(
									activeQuestTexts,
									string.format("Quest: %s [%d/%d]", v.name, v.progress, v.required)
								)
								local currentGems = util.data.stats["Gems"] or 0
								local requiredGems = diff * 50
								if requiredGems < 500 then
									requiredGems = 500
								end
						...
						elseif
							string.find(lowerName, "boss")
							or string.find(lowerName, "kill")
							or string.find(lowerName, "story")
							or string.find(lowerName, "any")
							or string.find(lowerName, "clear")
						then
			...
			local function runAutoCraft()
				...
				for matName, requiredPerCraftStr in pairs(recipe) do
					...
					if have < totalRequired then
						...
						table.insert(
							missingMats,
							{ name = matName, short = totalRequired - have, total = totalRequired }
						)
						if string.lower(matName) == "gold" then
							isMissingGold = true
						end
					end
				end
				...
				elseif totalMissingCount > 0 then
					...
					for i = 1, targetQty do
						local succ, res = pcall(function()
							return game:GetService("ReplicatedStorage").Remotes.Crafting.craft
								:InvokeServer(targetName, 1)
						end)
						if succ and res then
					...
					if util.data.characters then
						for id, charData in pairs(util.data.characters) do
							if charData.name == targetName then
								local succ, res = pcall(function()
									return game:GetService("ReplicatedStorage").Remotes.Awakening.awaken
										:InvokeServer(id)
								end)
								if succ and res then
			...
			local function runAutoReroll()
				...
				if _G.DEBUG_REROLL then
					print("[AutoReroll] Checking unit:", unit.name, "Potential:", unit.potential)
				end
				...
				if unit.potential and tonumber(unit.potential) >= 1000 then
					...
					if unit.ranks then
						for statName, rank in pairs(unit.ranks) do
							if _G.DEBUG_REROLL then
								print("[AutoReroll] Stat:", statName, "Rank:", rank)
							end
							if lockToggles[rank] then
								locksToApply[statName] = true
								if _G.DEBUG_REROLL then
									print("[AutoReroll] Locking:", statName)
								end
							else
								hasUnlock = true
							end
					...
					if _G.DEBUG_REROLL then
						print("[AutoReroll] hasUnlock:", hasUnlock)
					end
			...
			local priorities = {
				...
				Options.Priority5 and Options.Priority5.Value or "Auto Prism",
			}

			local handled = false
			local priorityDropFarm = nil
			local activeGearFarmTask = getActiveGearFarmTask()
			for _, p in ipairs(priorities) do
				if not handled then
					if p == "Auto Quest" then
						handled = runAutoQuest()
					elseif
						p == "Auto Farm Gear"
						and Options.AutoFarmGear
						and Options.AutoFarmGear.Value
						and activeGearFarmTask
					then
						local map = resolveFarmMap(gearFarmMaps, activeGearFarmTask.name, activeGearFarmTask.difficulty)
						if map then
							priorityDropFarm = { kind = "Gear", target = activeGearFarmTask.name, map = map }
							table.insert(activeQuestTexts, "Farm Gear: " .. activeGearFarmTask.name)
							handled = true
						end
					elseif p == "Auto Farm Unit" and Options.AutoFarmUnit and Options.AutoFarmUnit.Value then
						local map =
							resolveFarmMap(unitFarmMaps, Options.FarmUnitTarget.Value, Options.FarmUnitDifficulty.Value)
						if map then
							priorityDropFarm = { kind = "Unit", target = Options.FarmUnitTarget.Value, map = map }
							table.insert(activeQuestTexts, "Farm Unit: " .. Options.FarmUnitTarget.Value)
							handled = true
						end
					elseif p == "Auto Craft" then
						handled = runAutoCraft()
					elseif p == "Auto Evo" then
						handled = runAutoEvo()
					elseif p == "Auto Reroll" then
						handled = runAutoReroll()
					elseif p == "Auto Prism" and Options.AutoPrism and Options.AutoPrism.Value then
						local sortedPrism = getSortedValidPrismMaps()
						for _, data in ipairs(sortedPrism) do
							local currentCap = util and util.data and util.data.caps and util.data.caps[data.cfg.capStr]
								or 0
							if currentCap < data.cfg.mapData.cap then
								table.insert(activeQuestTexts, "Prism Farm: " .. data.cfg.mapData.world)
								handled = true
								break
							end
						end
			...
			then
				...
				if activeQuestMap then
					...
					elseif activeQuestMap.mode == "Story" then
						Fluent:Notify({
							Title = "Auto Quest",
							Content = "Joining GT City Act 1 for Quest!",
							Duration = 3,
						})
						joinRoom(
							activeQuestMap.act,
				...
				elseif priorityDropFarm then
					local map = priorityDropFarm.map
					Fluent:Notify({
						Title = "Auto Farm " .. priorityDropFarm.kind,
						Content = priorityDropFarm.target .. " | " .. formatFarmMap(map),
						Duration = 3,
					})
					joinRoom(map.act, map.difficulty, map.mode, map.world, nil, nil, nil)
				elseif handled then
				elseif
					succ
					and type(challengeData) == "table"
					and Options.AutoJoin1d.Value
					and challengeData["1d"]
					and not dailyCompleted
				then
					Fluent:Notify({ Title = "Sniper", Content = "Joining Daily Challenge!", Duration = 3 })
					local s, err = joinRoom(
						challengeData["1d"].act,
						"1d",
						"Challenge",
						challengeData["1d"].world,
						challengeData["1d"].rewards,
						nil,
						nil
					)
				...
				elseif shouldSnipe30m then
					...
					joinRoom(
						challengeData["30m"].act,
						"30m",
						"Challenge",
						challengeData["30m"].world,
						challengeData["30m"].rewards,
						nil,
						nil
					)
				elseif
					Options.AutoFarmGear
					and Options.AutoFarmGear.Value
					and activeGearFarmTask
					and resolveFarmMap(gearFarmMaps, activeGearFarmTask.name, activeGearFarmTask.difficulty)
				then
					local map = resolveFarmMap(gearFarmMaps, activeGearFarmTask.name, activeGearFarmTask.difficulty)
					Fluent:Notify({
						Title = "Auto Farm Gear",
						Content = activeGearFarmTask.name .. " | " .. formatFarmMap(map),
						Duration = 3,
					})
					joinRoom(map.act, map.difficulty, map.mode, map.world, nil, nil, nil)
				elseif
					Options.AutoFarmUnit
					and Options.AutoFarmUnit.Value
					and resolveFarmMap(unitFarmMaps, Options.FarmUnitTarget.Value, Options.FarmUnitDifficulty.Value)
				then
					local map =
						resolveFarmMap(unitFarmMaps, Options.FarmUnitTarget.Value, Options.FarmUnitDifficulty.Value)
					Fluent:Notify({
						Title = "Auto Farm Unit",
						Content = Options.FarmUnitTarget.Value .. " | " .. formatFarmMap(map),
						Duration = 3,
					})
					joinRoom(map.act, map.difficulty, map.mode, map.world, nil, nil, nil)
				elseif Options.AutoJoinMap and Options.AutoJoinMap.Value then
					local world = Options.AutoJoinWorld.Value
					local mode = Options.AutoJoinMode.Value
					local difficulty = Options.AutoJoinDifficulty.Value
					local act = tonumber(string.match(tostring(Options.AutoJoinAct.Value), "%d+"))
					local mapData = act
						and joinableMapTree[world]
						and joinableMapTree[world][mode]
						and joinableMapTree[world][mode][difficulty]
					...
					if mapData then
						...
						Fluent:Notify({
							Title = "Auto Join Map",
							Content = string.format("%s - Act %d (%s)", world, act, difficulty),
							Duration = 3,
						})
						joinRoom(
							act,
							mapData.omitDifficulty and nil or difficulty,
							mode,
							mapData.world or world,
							mapData.rewards,
							nil,
							nil,
							nil,
					...
					if targetData then
						...
						joinRoom(
							targetData.cfg.mapData.act,
							targetData.difficulty,
							targetData.cfg.mapData.mode,
							targetData.cfg.mapData.world,
							nil,
							targetData.cfg.capStr,
							targetData.cfg.mapData.cap
						)
					elseif Options.AutoPrism and Options.AutoPrism.Value then
						local prismData = nil
						local sortedPrism = getSortedValidPrismMaps()
						for _, data in ipairs(sortedPrism) do
							local currentCap = util and util.data and util.data.caps and util.data.caps[data.cfg.capStr]
						...
						if prismData then
							Fluent:Notify({
								Title = "Prism Farm",
								Content = "Joining: "
									.. prismData.cfg.mapData.world
									.. " ("
									.. prismData.difficulty
									.. ") Prism",
								Duration = 3,
							})
							joinRoom(
								prismData.cfg.mapData.act,
								prismData.difficulty,
								prismData.cfg.mapData.mode,
								prismData.cfg.mapData.world,
								nil,
								prismData.cfg.capStr,
								prismData.cfg.mapData.cap
							)
						end
					end
				end
	...
	if isfile and readfile and isfile(basePath .. "/CurrentTarget.json") then
		local succ, parsed = pcall(function()
			return HttpService:JSONDecode(readfile(basePath .. "/CurrentTarget.json"))
		end)
		if succ and parsed then
			...
			print(
				"[AutoFarm] Limit data loaded: "
					.. tostring(targetCapStr)
					.. " (Max: "
					.. tostring(targetMaxCap)
					.. ") Type: "
					.. tostring(targetCapType)
			)
		end
	end
			...
			while true do
				local success, err = pcall(function()
					...
					if teleportRemote then
						teleportRemote = teleportRemote:FindFirstChild("Players")
					end
					if teleportRemote then
						teleportRemote = teleportRemote:FindFirstChild("teleport")
					end
	...
	game.Players.PlayerRemoving:Connect(function(player)
		if
			Options.PartyMode
			and Options.PartyMode.Value
			and Options.PartyLeaveSync
			and Options.PartyLeaveSync.Value
		then
		...
		while true do
					...
					if v.name ~= "Complete All" and v.name ~= "Weekly Complete All" then
						if v.progress < v.required then
							...
							if
								...
								or string.find(lowerName, "any")
								or string.find(lowerName, "clear")
								or string.find(lowerName, "summon")
							then
								table.insert(
									activeQuestTexts,
									string.format("Quest: %s [%d/%d]", v.name, v.progress, v.required)
			...
			if targetCapStr and targetMaxCap and util and util.data then
				...
				if targetCapType == "Item" then
					currentVal = (util.data.items and util.data.items[targetCapStr] or 0)
						+ (util.data.stats and util.data.stats[targetCapStr] or 0)
				else
					currentVal = util.data.caps and util.data.caps[targetCapStr] or 0
				end
				table.insert(
					activeTexts,
					"Current Goal: " .. tostring(targetCapStr) .. " [" .. currentVal .. " / " .. targetMaxCap .. "]"
				)
			...
			if StatusParagraph then
				local fullText =
					"NOTE: Auto Farm functions will NOT operate while in-game. It will resume automatically in the Lobby."
			...
			if Options.AutoPlayToggle and Options.AutoPlayToggle.Value and os.time() - _lastAutoPlayRequest >= 10 then
				...
				pcall(function()
					if util and util.data and not util.data.autoplay then
						...
						if Event then
							Event = Event:FindFirstChild("Characters")
						end
						if Event then
							Event = Event:FindFirstChild("autoplay")
						end
			...
			if not isTeleporting and Options.AutoLeaveBaseFailsafe and Options.AutoLeaveBaseFailsafe.Value and _ingameTick % 5 == 0 then
				...
				if baseDead and not hasEndScreen then
					...
					elseif os.time() - baseZeroTime >= 10 then
						forceTeleportToLobby(
							"Failsafe Abort",
							"Base reached 0 HP (waited 10s, no EndScreen)! Teleporting to Lobby..."
						)
						baseZeroTime = nil
					end
				else
			...
			if
				not isTeleporting
				and Options.AutoSniperSync
				and Options.AutoSniperSync.Value
	...
	local statsCountedForMatch = false
		...
		while true do
			...
			if menus then
				...
				if endScreen and endScreen.Visible then
					if not statsCountedForMatch then
						statsCountedForMatch = true
						...
						for _, child in pairs(endScreen:GetDescendants()) do
							if child:IsA("TextLabel") then
								...
								if
									string.find(text, "defeat")
									or string.find(text, "lose")
									or string.find(text, "fail")
								then
									matchStatus = "LOSS"
									break
								end
						...
						if matchStatus == "LOSS" then
							...
							pcall(function()
								forceTeleportToLobby("Match Lost", L.AF_MatchLost)
							end)
						end
					end
					...
					if
						not isTeleporting
						and Options.AutoQuest
						and Options.AutoQuest.Value
						and startedWithIncompleteQuest
						and util
						and util.data
						and util.data.quests
					then
						...
						if not hasIncompleteQuest then
							forceTeleportToLobby(
								"Quest Completed!",
								"All farmable quests finished! Teleporting to Lobby..."
							)
						end
					end
					...
					if
						not isTeleporting
						and Options.AutoLeaveToggle.Value
						and targetCapStr
						and targetMaxCap
						and util
						and util.data
					...
					local previousGearTask = _G.AnimeSquadron_GearFarmQueue[1]
					local currentGearTask = Options.AutoFarmGear
							and Options.AutoFarmGear.Value
							and getActiveGearFarmTask()
						or nil
					if previousGearTask and currentGearTask ~= previousGearTask then
						forceTeleportToLobby(
							"Auto Farm Gear",
							"Gear target completed! Returning to lobby for the next task..."
						)
					elseif Options.AutoFarmGear and Options.AutoFarmGear.Value and not currentGearTask then
						AutoFarmGear:SetValue(false)
					end
					local farmingDrop = currentGearTask ~= nil or (Options.AutoFarmUnit and Options.AutoFarmUnit.Value)
					if not isTeleporting and (Options.AutoReplayToggle.Value or farmingDrop) and replayEvent then
	...
	while true do
		...
		if util and util.data and util.data.stats then
			...
			for _, k in ipairs(statKeys) do
				if util.data.stats[k] then
					currentItems[k] = util.data.stats[k]
				end
			end

			if lastItems ~= nil then
