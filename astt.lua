                ...
                if type(difficulties) == "table" then
                    ...
                    for difficulty, acts in pairs(difficulties) do
                        if type(acts) == "table" then
                            joinableMapTree[world.name][mode][difficulty] = joinableMapTree[world.name][mode][difficulty] or {}
...
local crusaderLoaded, Crusader = pcall(function()
    return require(Players.LocalPlayer.PlayerScripts.Client.Events.Crusader)
end)
if crusaderLoaded and type(Crusader) == "table" and type(Crusader.rewards) == "table" then
    joinableMapTree["Crusader Event"] = { Event = { Normal = {} } }
    for act, rewards in ipairs(Crusader.rewards) do
        joinableMapTree["Crusader Event"].Event.Normal[act] = {
            world = "Crusader Desert",
            omitDifficulty = true,
            rewards = rewards,
            boss = Crusader.bosses and Crusader.bosses[act]
        }
    end
end
...
for target in pairs(gearFarmMaps) do table.insert(gearFarmTargets, target) end
for target in pairs(unitFarmMaps) do table.insert(unitFarmTargets, target) end
...
local Locales = {
    EN = {
        ...
        TabAutoFarm = "Auto Farm", TabAutoJoinMap = "Auto Join Map", TabFarmGear = "Farm Gear", TabFarmUnit = "Farm Unit", TabPriority = "Priority Settings", TabMaps = "Trait Maps", TabEvo = "Evo & Craft", TabReroll = "Auto Reroll", TabShop = "Shop & Upgrades", TabClaims = "Claims & Misc", TabSniper = "Challenge Sniper", TabIngame = "Ingame Helper", TabParty = "Party & Multi", TabSettings = "Settings",
        ...
        AF_AutoMap = "Enable Auto Join Map", AF_AutoMapD = "Automatically joins the selected map when higher-priority tasks are idle.",
        AF_MapWorld = "Story / World", AF_MapWorldD = "Select a story world.",
        AF_MapMode = "Mode", AF_MapModeD = "Available modes for the selected world.",
        AF_MapDiff = "Difficulty", AF_MapDiffD = "Available difficulties for the selected mode.",
        AF_MapAct = "Map / Act", AF_MapActD = "Available maps in the selected story.",
        FarmGearTarget = "Target Gear", FarmGearTargetD = "Select gear; its drop map is detected automatically.",
        FarmGearQty = "Quantity to Farm", FarmGearQtyD = "Number of new copies to obtain.",
        FarmGearQueue = "Gear Farm Queue", FarmGearQueueD = "No gear queued.",
        FarmGearAdd = "Add to Queue", FarmGearAddD = "Add the selected gear, quantity, and difficulty.",
        FarmGearClear = "Clear Queue", FarmGearClearD = "Remove all queued gear.",
        FarmUnitTarget = "Target Unit", FarmUnitTargetD = "Select a drop unit; its map is detected automatically.",
        FarmDifficulty = "Farm Difficulty", FarmDifficultyD = "Hard usually has a higher drop chance.",
        FarmGearToggle = "Enable Auto Farm Gear", FarmGearToggleD = "Automatically joins and replays the selected gear map.",
        FarmUnitToggle = "Enable Auto Farm Unit", FarmUnitToggleD = "Automatically joins and replays the selected unit map.",
        FarmTargetInfo = "Target Map", FarmTargetWaiting = "Select a valid target.",
    ...
    VN = {
        WinTitle = "🇻🇳 Free HUB", WinSub = "Anime Squadron",
        TabAutoFarm = "Auto Farm", TabAutoJoinMap = "Tự động Vào Map", TabFarmGear = "Farm Gear", TabFarmUnit = "Farm Unit", TabPriority = "Cài đặt Ưu tiên", TabMaps = "Bản đồ Trait", TabEvo = "Tiến hoá & Ghép", TabReroll = "Auto Đập Chỉ số", TabShop = "Cửa hàng & Nâng cấp", TabClaims = "Nhận thưởng & Code", TabSniper = "Săn Challenge", TabIngame = "Hỗ trợ Trong Game", TabParty = "Tổ đội (Nhiều Acc)", TabSettings = "Cài đặt (Settings)",
        ...
        AF_AutoMap = "Bật Tự động Vào Map", AF_AutoMapD = "Tự vào map đã chọn khi các tác vụ ưu tiên cao hơn đang rảnh.",
        AF_MapWorld = "Story / World", AF_MapWorldD = "Chọn khu vực story.",
        AF_MapMode = "Chế độ", AF_MapModeD = "Các chế độ có trong story đã chọn.",
        AF_MapDiff = "Độ khó", AF_MapDiffD = "Các độ khó có trong chế độ đã chọn.",
        AF_MapAct = "Map / Act", AF_MapActD = "Danh sách map thuộc story đã chọn.",
        FarmGearTarget = "Gear cần Farm", FarmGearTargetD = "Chọn gear, script tự tìm map rơi gear đó.",
        FarmGearQty = "Số lượng cần Farm", FarmGearQtyD = "Số bản gear mới cần kiếm.",
        FarmGearQueue = "Hàng đợi Farm Gear", FarmGearQueueD = "Chưa có gear trong hàng đợi.",
        FarmGearAdd = "Thêm vào Hàng đợi", FarmGearAddD = "Thêm gear, số lượng và độ khó đã chọn.",
        FarmGearClear = "Xoá Hàng đợi", FarmGearClearD = "Xoá toàn bộ gear đang chờ farm.",
        FarmUnitTarget = "Unit cần Farm", FarmUnitTargetD = "Chọn unit rơi từ map, script tự tìm đúng map.",
        FarmDifficulty = "Độ khó Farm", FarmDifficultyD = "Hard thường có tỉ lệ rơi cao hơn.",
        FarmGearToggle = "Bật Auto Farm Gear", FarmGearToggleD = "Tự vào và replay map rơi gear đã chọn.",
        FarmUnitToggle = "Bật Auto Farm Unit", FarmUnitToggleD = "Tự vào và replay map rơi unit đã chọn.",
        FarmTargetInfo = "Map Mục tiêu", FarmTargetWaiting = "Hãy chọn mục tiêu hợp lệ.",
...
local AutoJoinMode = Tabs.AutoJoinMap:AddDropdown("AutoJoinMode", { Title = L.AF_MapMode, Description = L.AF_MapModeD, Values = {"Story"}, Multi = false, Default = 1 })
local AutoJoinDifficulty = Tabs.AutoJoinMap:AddDropdown("AutoJoinDifficulty", { Title = L.AF_MapDiff, Description = L.AF_MapDiffD, Values = {"Normal"}, Multi = false, Default = 1 })
local AutoJoinAct = Tabs.AutoJoinMap:AddDropdown("AutoJoinAct", { Title = L.AF_MapAct, Description = L.AF_MapActD, Values = {"Act 1"}, Multi = false, Default = 1 })
local AutoJoinMap = Tabs.AutoJoinMap:AddToggle("AutoJoinMap", { Title = L.AF_AutoMap, Description = L.AF_AutoMapD, Default = false })
...
local function refreshAutoJoinMapControls()
    if refreshingAutoJoinMap then return end
    ...
    if not worldData[Options.AutoJoinMode.Value] then AutoJoinMode:SetValue(modes[1] or "(None)") end
    ...
    if not modeData[Options.AutoJoinDifficulty.Value] then AutoJoinDifficulty:SetValue(difficulties[1] or "(None)") end

    local difficultyData = modeData[Options.AutoJoinDifficulty.Value] or {}
    local acts = sortedKeys(difficultyData, function(act) return "Act " .. tostring(act) end)
    AutoJoinAct:SetValues(#acts > 0 and acts or {"(None)"})
    local currentAct = tonumber(string.match(tostring(Options.AutoJoinAct.Value), "%d+"))
    if not currentAct or not difficultyData[currentAct] then AutoJoinAct:SetValue(acts[1] or "(None)") end
...
local FarmGearDifficulty = Tabs.FarmGear:AddDropdown("FarmGearDifficulty", { Title = L.FarmDifficulty, Description = L.FarmDifficultyD, Values = {"Hard", "Normal"}, Multi = false, Default = 1 })
local FarmGearQty = Tabs.FarmGear:AddInput("FarmGearQty", { Title = L.FarmGearQty, Description = L.FarmGearQtyD, Default = "1", Numeric = true, Finished = false })
...
local function getOwnedGearCount(targetName)
    ...
    for _, gear in pairs(util and util.data and util.data.gear or {}) do
        if type(gear) == "table" and gear.name == targetName then count = count + 1 end
    end
    return count
end
...
if isfile and readfile and isfile(gearFarmQueueFile) then
    pcall(function()
        ...
        if type(data) == "table" then _G.AnimeSquadron_GearFarmQueue = data end
    end)
end

local function saveGearFarmQueue()
    if writefile then
        pcall(function() writefile(gearFarmQueueFile, HttpService:JSONEncode(_G.AnimeSquadron_GearFarmQueue)) end)
    end
end
...
local function updateGearFarmQueueUI()
    ...
    for i, taskData in ipairs(_G.AnimeSquadron_GearFarmQueue) do
        ...
        table.insert(lines, string.format("%d. %s x%d [%d/%d] (%s)", i, taskData.name, taskData.qty, gained, taskData.qty, taskData.difficulty))
    end
...
Tabs.FarmGear:AddButton({
    ...
    Callback = function()
        ...
        if not gearFarmMaps[target] then return end
...
local function getActiveGearFarmTask()
    ...
    while _G.AnimeSquadron_GearFarmQueue[1] do
        ...
        if owned - taskData.startCount >= taskData.qty then
            ...
            if changed then saveGearFarmQueue() end
            updateGearFarmQueueUI()
            return taskData
        end
    end
...
updateGearFarmQueueUI()
local AutoFarmGear = Tabs.FarmGear:AddToggle("AutoFarmGear", { Title = L.FarmGearToggle, Description = L.FarmGearToggleD, Default = false })
...
local FarmUnitDifficulty = Tabs.FarmUnit:AddDropdown("FarmUnitDifficulty", { Title = L.FarmDifficulty, Description = L.FarmDifficultyD, Values = {"Hard", "Normal"}, Multi = false, Default = 1 })
local FarmUnitInfo = Tabs.FarmUnit:AddParagraph({ Title = L.FarmTargetInfo, Content = L.FarmTargetWaiting })
local AutoFarmUnit = Tabs.FarmUnit:AddToggle("AutoFarmUnit", { Title = L.FarmUnitToggle, Description = L.FarmUnitToggleD, Default = false })
...
local function formatFarmMap(map)
    if not map then return L.FarmTargetWaiting end
...
local function refreshFarmTargetInfo()
    FarmGearInfo:SetDesc(formatFarmMap(resolveFarmMap(gearFarmMaps, Options.FarmGearTarget.Value, Options.FarmGearDifficulty.Value)))
    FarmUnitInfo:SetDesc(formatFarmMap(resolveFarmMap(unitFarmMaps, Options.FarmUnitTarget.Value, Options.FarmUnitDifficulty.Value)))
end
...
local PriorityList = {"Auto Quest", "Auto Farm Gear", "Auto Farm Unit", "Auto Craft", "Auto Evo", "Auto Reroll", "None"}
...
for name, option in pairs(Fluent.Options) do
    option:OnChanged(function()
        if name == "AutoJoinWorld" or name == "AutoJoinMode" or name == "AutoJoinDifficulty" then
            refreshAutoJoinMapControls()
        elseif name == "FarmGearTarget" or name == "FarmGearDifficulty" or name == "FarmUnitTarget" or name == "FarmUnitDifficulty" then
            refreshFarmTargetInfo()
        end
...
if isLobby then
    ...
    local function joinRoom(act, diff, mode, world, rewards, capStr, maxCap, capType, boss)
        ...
        if diff then roomData.difficulty = diff end
        ...
        while true do
            ...
            if Options.MasterAutoRun.Value then
                if Options.StuckAutoHop and Options.StuckAutoHop.Value then
                    ...
                    if os.time() - lobbyEnterTime > timeout then
                        ...
                        pcall(function()
                            local body = game:HttpGet("https://games.roblox.com/v1/games/" .. game.PlaceId .. "/servers/Public?sortOrder=Desc&limit=100")
            ...
            for _, p in ipairs(priorities) do
                if not handled then
                    ...
                    elseif p == "Auto Farm Gear" and Options.AutoFarmGear and Options.AutoFarmGear.Value and activeGearFarmTask then
                    ...
                    elseif p == "Auto Farm Unit" and Options.AutoFarmUnit and Options.AutoFarmUnit.Value then
                        local map = resolveFarmMap(unitFarmMaps, Options.FarmUnitTarget.Value, Options.FarmUnitDifficulty.Value)
                        if map then
            ...
            if Options.MasterAutoRun.Value and get_challenges and create_room and not (Options.PartyMode and Options.PartyMode.Value and Options.PartyRole.Value == "Member") then
                ...
                elseif priorityDropFarm then
                    ...
                    Fluent:Notify({ Title = "Auto Farm " .. priorityDropFarm.kind, Content = priorityDropFarm.target .. " | " .. formatFarmMap(map), Duration = 3 })
                ...
                elseif Options.AutoFarmGear and Options.AutoFarmGear.Value and activeGearFarmTask and resolveFarmMap(gearFarmMaps, activeGearFarmTask.name, activeGearFarmTask.difficulty) then
                    local map = resolveFarmMap(gearFarmMaps, activeGearFarmTask.name, activeGearFarmTask.difficulty)
                    Fluent:Notify({ Title = "Auto Farm Gear", Content = activeGearFarmTask.name .. " | " .. formatFarmMap(map), Duration = 3 })
                    joinRoom(map.act, map.difficulty, map.mode, map.world, nil, nil, nil)
                elseif Options.AutoFarmUnit and Options.AutoFarmUnit.Value and resolveFarmMap(unitFarmMaps, Options.FarmUnitTarget.Value, Options.FarmUnitDifficulty.Value) then
                    local map = resolveFarmMap(unitFarmMaps, Options.FarmUnitTarget.Value, Options.FarmUnitDifficulty.Value)
                    Fluent:Notify({ Title = "Auto Farm Unit", Content = Options.FarmUnitTarget.Value .. " | " .. formatFarmMap(map), Duration = 3 })
                    joinRoom(map.act, map.difficulty, map.mode, map.world, nil, nil, nil)
                elseif Options.AutoJoinMap and Options.AutoJoinMap.Value then
                    ...
                    local mapData = act and joinableMapTree[world] and joinableMapTree[world][mode] and joinableMapTree[world][mode][difficulty] and joinableMapTree[world][mode][difficulty][act]
                    if mapData then
                        Fluent:Notify({ Title = "Auto Join Map", Content = string.format("%s - Act %d (%s)", world, act, difficulty), Duration = 3 })
                        joinRoom(act, mapData.omitDifficulty and nil or difficulty, mode, mapData.world or world, mapData.rewards, nil, nil, nil, mapData.boss)
                    end
                else
        ...
        while true do
            ...
            if menus then
                ...
                if endScreen and endScreen.Visible then
                    ...
                    local currentGearTask = Options.AutoFarmGear and Options.AutoFarmGear.Value and getActiveGearFarmTask() or nil
                    if previousGearTask and currentGearTask ~= previousGearTask then
                        forceTeleportToLobby("Auto Farm Gear", "Gear target completed! Returning to lobby for the next task...")
